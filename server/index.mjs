...
function bindRoomBroadcasts(room) {
  ...
  room.on('playerJoined', (player) => {
    try {
      ...
      try {
        io.emit('lobbyRooms', getLobbyRooms());
      } catch (e) {
...
// bind existing rooms
for (const room of appInstance.lobby.rooms.values()) bindRoomBroadcasts(room);
// bind future rooms
appInstance.lobby.on('roomAdded', (room) => {
  bindRoomBroadcasts(room);
...
io.on('connection', (socket) => {
  ...
  try {
    socket.emit('lobbyRooms', getLobbyRooms());
  } catch (e) {
